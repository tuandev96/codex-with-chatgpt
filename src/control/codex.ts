...
export interface CodexRunInput {
  ...
  iteration: number;
  prompt: string;
  /** Which local coding agent to dispatch. Defaults to codex. */
  agent?: AgentId;
...
export interface CodexControl {
  ...
  listAgents?(): { id: string; displayName: string; installed: boolean; command: string | null }[];
}
...
export interface CodexExecutorOptions {
  /** Override the binary for the default (codex) agent; tests use commandPrefix. */
  command?: string;
  /** Per-agent binary overrides, e.g. { cursor: "/path/to/cursor-agent" }. */
  agentCommands?: Partial<Record<AgentId, string>>;
  /** Test-only argv prefix; production invokes the agent binary directly. */
...
function validateRunInput(input: CodexRunInput): void {
  ...
  if (input.agent !== undefined && getAdapter(input.agent) === null) {
    throw new CodexControlError("INVALID_CONTROL_REQUEST", `Unknown agent '${input.agent}'. Call agents_list first.`);
  }
  if (
...
async function waitForProcess(
  ...
  const result = await new Promise<CapturedProcess>((resolve) => {
    ...
    child.once("error", (error) => {
      logger.error("Worker process failed", { message: error.message });
      finish(null, null);
    });
    child.once("close", (exitCode, signal) => finish(exitCode, signal));
    timer = setTimeout(() => {
      timedOut = true;
      logger.warn("Worker timed out", { timeoutMs });
      child.kill("SIGTERM");
...
export class CodexExecutor implements CodexControl {
  ...
  async run(input: CodexRunInput): Promise<CodexRunResult> {
    ...
    const agent: AgentId = input.agent ?? "codex";
    const adapter = getAdapter(agent)!;
    const key = createHash("sha256").update(JSON.stringify([agent, input.taskId, input.iteration])).digest("hex");
    const fingerprint = createHash("sha256").update(JSON.stringify([
      agent, input.prompt, input.resumeThreadId ?? null, input.model ?? null, input.sandbox, input.timeoutMs,
    ])).digest("hex");
    ...
    // One worker per workspace; use a queue only if parallel execution is explicitly needed.
    try {
      fs.writeFileSync(lock, JSON.stringify({ agent, taskId: input.taskId, iteration: input.iteration, pid: process.pid }), { flag: "wx", mode: 0o600 });
    } catch (error) {
    ...
    const running: CodexRunResult = {
      taskId: input.taskId, iteration: input.iteration, agent, status: "running", exitCode: null,
    ...
    const pending = Promise.resolve().then(() => this.execute(adapter, agent, input)).catch((error: unknown): CodexRunResult => ({
  ...
  private resolveCommand(agent: AgentId): string {
    const override = this.options.agentCommands?.[agent];
    if (override) return override;
    if (agent === "codex" && this.options.command) return this.options.command;
    return resolveAgentCommand(agent);
  }
  ...
  private async execute(
    ...
    const workerPrompt = `${adapter.workerPrefix}\n\nCoordinator task:\n${input.prompt.trim()}`;
    ...
    const command = this.resolveCommand(agent);
    const spawnProcess = this.options.spawnProcess ?? spawn;
    let captured: CapturedProcess;
    try {
      const child = spawnProcess(command, [...(this.options.commandPrefix ?? []), ...plan.args], {
        cwd: executionRoot,
        env: process.env,
      ...
      throw new CodexControlError(
        ...
        `Unable to start ${adapter.displayName}: ${error instanceof Error ? error.message : String(error)}`
      );
    }

    try {
      // For adapters that take the prompt as argv, rebuild args with the prefixed prompt.
      const parsed = adapter.parseOutput(captured.stdout, input.resumeThreadId);
      ...
      const output = saveExecutionOutput(this.workspace.id, {
        command: `${adapter.displayName} worker (ChatGPT coordinator)`,
        raw: rawOutput,
      ...
      this.logger.error("Unable to persist worker execution evidence", { message: String(error) });
      throw error;
    }
  }
}

/** Back-compat alias. Prefer CodexExecutor; the control plane now supports multiple agents. */
export const AgentExecutor = CodexExecutor;
export { codexAdapter };
